...
static const int sizes[19] = {16,64,256,1024,1024,32,32,128,128,512,512,1024,1024,64,64,256,256,512,512};
int main(void){
  printf("//! AUTO-GENERATED scan-order tables from dav1d `src/scan.c` (BSD-2-Clause,\n");
  printf("//! (c) VideoLAN). `SCANS[tx]` = raster coefficient positions in scan order.\n");
  printf("//! Regenerate via tools/extract_av1_cdf/generate.sh. DO NOT EDIT.\n");
  printf("#![allow(dead_code)]\n#![allow(clippy::all)]\n\n");
  for(int i=0;i<19;i++){
    printf("static SCAN_%d: [u16; %d] = [", i, sizes[i]);
    for(int j=0;j<sizes[i];j++) printf("%u,", dav1d_scans[i][j]);
    printf("];\n");
  }
  printf("\npub(crate) static SCANS: [&[u16]; 19] = [");
  for(int i=0;i<19;i++) printf("&SCAN_%d,", i);
  printf("];\n");
  return 0;
}
