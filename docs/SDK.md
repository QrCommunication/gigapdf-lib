...
| `imageToPdf(image)` | `Uint8Array` | Wrap a raster image in a single A4-page PDF (centred, shrink-to-fit, never upscaled). Format auto-detected: **PNG, JPEG, GIF, WebP, AVIF, TIFF** (GIF/WebP/AVIF/TIFF transcoded to PNG before embed; PNG keeps every color-type & bit-depth, Adam7 interlacing and transparency via `/SMask`). Empty `Uint8Array` for an unrecognized format. |
...
| `resizePage(page, width, height)` | `boolean` | Set the page MediaBox to `width`×`height` points — **box geometry only** (the content keeps its coordinates and is *not* rescaled). Use `scalePageContent`/`scalePageTo` to scale the content too. |
| `scalePageContent(page, factor)` | `boolean` | **Scale the page content** uniformly by `factor` about the origin (ISO 32000-1 §8.3.4) — a true page zoom: wraps the content in `q <factor 0 0 factor 0 0> cm … Q`, scales `/MediaBox`+`/CropBox` (and any declared Bleed/Trim/Art), **and** scales every annotation `/Rect` so widget/stamp appearances follow (mapped `/BBox`→`/Rect` per §12.5.5). `factor` must be finite and positive. |
| `scalePageContentXY(page, sx, sy)` | `boolean` | Anisotropic `scalePageContent`: scale content by `sx` horizontally, `sy` vertically (boxes and annotation rects too). |
| `scalePageTo(page, width, height)` | `number \| null` | Scale the content to **fit within** `width`×`height` points (shrink-/grow-to-fit), aspect preserved — `min(width/curW, height/curH)` over the displayed size (`/MediaBox` with `/Rotate`). Returns the uniform factor applied, or `null` on error. |
| `setUserUnit(page, unit)` | `boolean` | Set the page `/UserUnit` (ISO 32000-1 §14.11.2) for **large-format authoring**: one default user-space unit becomes `unit`⁄72 inch (lifts the ~200-inch coordinate ceiling). `1.0` (the default) removes the key. Distinct from content scaling — it rescales the *coordinate system*, not the content/boxes. `unit` must be finite and positive. |
| `pageUserUnit(page)` | `number \| null` | The page's `/UserUnit` (default `1.0` when absent); `null` on error. |
...
| `placePage(target, source, x, y, scaleX, scaleY)` | `boolean` | **N-up imposition** (ISO 32000-1 §8.10): draw the 1-based `source` page's content, scaled and translated, onto the 1-based `target` page as a Form XObject (its content + `/Resources`, so fonts/images come along). `(x, y)` is where the visible page's lower-left lands; `(scaleX, scaleY)` scale it **as displayed** (source `/MediaBox` origin & `/Rotate` absorbed into the matrix). Wrapped in `q`/`Q` — never disturbs existing target content, **composable** (call repeatedly for 2-up/4-up). Same page allowed. |
| `placePageMatrix(target, source, [a,b,c,d,e,f])` | `boolean` | Low-level primitive behind `placePage` with an explicit content-stream matrix (shear/arbitrary rotation/reflection). Maps the source's form space (its user space incl. `/MediaBox` origin) to the target; no origin/rotation normalisation — identity draws 1:1 at the target origin. |
| `nUp(cols, rows, opts?)` | `number` | Impose **all** existing pages `cols × rows` per sheet onto freshly added sheets (2-up/4-up/booklet thumbnails/contact sheets). Each page is scaled to fit its cell (aspect preserved) and centred; the originals are **dropped**, leaving only the imposed sheets. Returns the number of sheets. `opts`: `{ sheetWidth?, sheetHeight?, margin?, gutter? }` (default A4 portrait, 14 pt margin/gutter). |
...
| `figureCount()` | `number` | How many taggable **figures** the engine reconstructs across the whole document — the valid range `0..figureCount()` for `setFigureAlt`'s `index`. The accessible exports (`toTagged`, `toPdfA("pdfa-1a"\|"pdfa-2a")`) emit one `/Figure` structure element per figure, in this same order. |
| `setFigureAlt(index, alt)` | `boolean` | Attach **author-supplied alternate text** (`/Alt`, ISO 32000-1 §14.9.3) to a figure for the accessible (Tagged-PDF / PDF-UA) export, so assistive technology can describe the image (WCAG). `index` is the **document-global figure index** — figures numbered `0, 1, 2, …` in page order then page-content order (the Nth image of the whole document is figure `N`; bounded by `figureCount()`). This is a *different* index from `replaceImage`'s unified element index — it counts only the reconstructed figures, document-wide. The text lands on the matching `/Figure` `/Alt` in `toTagged`/`toPdfA("pdfa-1a"\|"pdfa-2a")`; figures with no author text keep a non-empty placeholder so the output stays structurally valid. `false` if `alt` is empty. |
...
| `toTagged({ pdfUa? })` | tagged (accessible) PDF — `/StructTreeRoot` + marked content + `/MarkInfo`/`/Lang`/`/RoleMap`/`/Alt`, without PDF/A; `pdfUa` adds the PDF/UA-1 identifier. Figures carry the author `/Alt` from `setFigureAlt` when set, else a placeholder |
