...
| `position` | `static`, `relative`, `absolute`, `fixed`, `sticky` | `relative` shifts by `inset`; `absolute` is placed by `inset` against the nearest positioned ancestor; `fixed` against the page box; **`sticky`** in a single-page block clamps the offset to the block, and in a **multi-page** block becomes a running **header** (`top` inset) or **footer** (`bottom` inset) — repeated at that edge on every page the block spans (the paged-media model: print has no scroll) |
