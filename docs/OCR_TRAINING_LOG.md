...
| 17 | 40 ep, **`kor` (new)** 32/64/128, **1487-class** (Hangul syllables + ASCII) | val_CER **0.749** | First Korean model — **weak/preliminary**: ~2.4k precomposed Hangul syllables are individually rare in 60k samples, and the synthetic-OCR source carries render artifacts. The real fix is **jamo decomposition** (~70 base letters → recompose at runtime), a future charset redesign; shipped as a baseline. |
