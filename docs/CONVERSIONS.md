...
| **EPUB** | Full (EPUB 3) | valid OCF, per-block XHTML (same fidelity as HTML export), embedded images, metadata, nav + NCX | **flat TOC** (one chapter per `Section`, depth 1), non-unique identifier, inline-only CSS; vector `Shape` still a 1em bordered box (the inline-`<svg>` shape rendering lands on the HTML path; the EPUB XHTML writer is the next step) |
