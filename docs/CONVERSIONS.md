...
| **PPTX / ODP** | Good | slides, text boxes, shapes, images, runs (bold/italic/colour), alignment; PPTX emits a complete OPC layout chain — every slide references a `slideLayout` → `slideMaster` → `theme` (opens without a PowerPoint *repair* prompt) | paragraph spacing/indent/line-height (PPTX), lists & tables flattened to paragraphs, run highlight, external links, super/sub, **speaker notes**, image format (PNG only) |
