...
| **DOC / XLS / PPT** (legacy OLE2) | **Text only** | flat plain text (largest stream, UTF‑16/ASCII) | **everything else** — styling, tables, sheets, slides, images, structure. The shared **CFB/OLE2 container reader** is now in place (`convert::cfb`, see note below); the per-format binary record parsers (`.doc` FIB/piece-table, `.xls` BIFF, `.ppt` records) that turn those streams into the rich model are the next step (tracked) |
