...
/// Default DCT-token probabilities `[plane][band][ctx][token]` (RFC 6386 §13.5).
...
/// Per-frame coeff-probability update flags `[plane][band][ctx][token]` (§13.4).
...
/// Keyframe 4×4 intra sub-block mode context probabilities `[above][left][mode]` (§11.2).
